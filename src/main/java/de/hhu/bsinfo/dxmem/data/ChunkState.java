...
/**
 * Different states for a chunk or data structure. DXRAM operations set the state of a chunk/data structure to indicate
 * failure/success and allow the user to handle errors
 *
...
public enum ChunkState {
    ...
    /**
     * Data for the chunk/data structure was temporary unavailable on the last operation (migration or recovery in
     * progress)
     */
    ...
    /**
     * The chunk operation included a lock request which failed to execute due to a user defined timeout
     * Typically, the user would simply retry the request including the operation until it succeeds
     */
    LOCK_TIMEOUT,

    /**
     * Chunk data had to be requested from a remote node and the request timed out (e.g. due to high load on remote).
