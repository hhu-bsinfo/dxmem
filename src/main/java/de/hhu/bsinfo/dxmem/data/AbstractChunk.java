 ...
 */
public abstract class AbstractChunk implements Importable, Exportable {
     ...
     *         Chunk ID to assign
     */
    public AbstractChunk(final long p_chunkID) {
        m_id = p_chunkID;
    }
    ...
    public boolean isIDValid() {
        return m_id != ChunkID.INVALID_ID;
    }
    ...
    /**
     * Check if the current state of the chunk is ok
     *
    ...
    /**
     * Method to convert the chunk IDs of a list of chunks to a list of hex strings
     *
     * @param p_chunks
     *         Chunks to generate a list of chunk IDs of
     * @return String with list of chunk IDs in hex
     */
    public static String toChunkIDListString(final AbstractChunk[] p_chunks) {
        StringBuilder builder = new StringBuilder();

        builder.append('[');

        for (int i = 0; i < p_chunks.length; i++) {
            builder.append(ChunkID.toHexString(p_chunks[i].getID()));

            if (i + 1 < p_chunks.length) {
                builder.append(", ");
            }
        }
     ...
     * @return String with list of chunk IDs in hex
     */
    public static String toSizeListString(final AbstractChunk[] p_chunks) {
        StringBuilder builder = new StringBuilder();

        builder.append('[');

        for (int i = 0; i < p_chunks.length; i++) {
            builder.append(p_chunks[i].sizeofObject());
