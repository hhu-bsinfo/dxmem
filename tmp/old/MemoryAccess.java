...
import de.hhu.bsinfo.dxmem.old.manipulation.ByteDataManipulation;
import de.hhu.bsinfo.dxmem.old.manipulation.ChunkDataManipulationTesting;
import de.hhu.bsinfo.dxmem.old.manipulation.DataStructureManipulation;
...
public class MemoryAccess {
    ...
    public boolean get(final DataStructure p_dataStructure, final boolean p_acquireWriteLock) {
        ...
        } else if ((directEntryAddress = cidTable.getAddressOfEntry(chunkID)) !=
                Heap.INVALID_ADDRESS) { //Check if a CID exist
            ...
            if (gotLock) { //check if a lock was received
                try {
                    ...
                    if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                        assert heap.getSizeDataBlock(entry) == p_dataStructure.sizeofObject();
     ...
     * @return A byte array with payload if getting the chunk payload
    ...
    public byte[] get(final long p_chunkID) {
        ...
        if (p_chunkID != ChunkID.INVALID_ID && //Check if CID can be correct
                (directEntryAddress = cidTable.getAddressOfEntry(p_chunkID)) != Heap.INVALID_ADDRESS &&
            ...
            try {
                ...
                entry = cidTable.directGet(directEntryAddress);
                if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                    int chunkSize = heap.getSizeDataBlock(entry);
                    ...
                    if (retSize != chunkSize) {
                        ...
                        throw new RuntimeException(
                                "Unknown error, importer size " + retSize + " != chunk size " + chunkSize);//<<
                    }
                }
    ...
    public byte[] get(final long p_chunkID, final boolean p_acquireWriteLock) {
        ...
        if ((directEntryAddress = cidTable.getAddressOfEntry(p_chunkID)) !=
                Heap.INVALID_ADDRESS) { //Check if a CID exist
            if (p_acquireWriteLock) {
                gotLock = memory.switchableWriteLock(directEntryAddress);
            } else {
        ...
        if (gotLock) { //check if a lock was received
            try {
                ...
                if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                    ...
                    if (memory.readLockDisabled() && ADDRESS.get(entry) >= heap.m_baseFreeBlockList) {
                        memory.switchableReadUnlock(directEntryAddress);
                        return null;
                    }
    ...
    public int get(final long p_chunkID, final byte[] p_buffer, final int p_offset, final int p_bufferSize,
            final boolean p_acquireWriteLock) {
        long directEntryAddress;
        ...
        //Check if a CID exist
        if ((directEntryAddress = cidTable.getAddressOfEntry(p_chunkID)) != Heap.INVALID_ADDRESS) {
            return -1;
        }
                ...
                if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                    ...
                    if (p_offset + chunkSize > p_bufferSize) {
                        ...
                        if (ret != chunkSize) {
                            ...
                            throw new RuntimeException(
                                    "Unknown error, importer size " + ret + " != chunk size " + chunkSize);//<<
                        }
                    }
                }
                // #ifdef STATISTICS
     ...
     * @return True if putting the data was successful, false if
    ...
    public boolean put(final DataStructure p_dataStructure, final boolean p_acquireWriteLock) {
        ...
        if (p_dataStructure.getID() == ChunkID.INVALID_ID) { //Check if CID can be correct
            p_dataStructure.setState(ChunkState.INVALID_ID);
        } else if ((directEntryAddress = cidTable.getAddressOfEntry(p_dataStructure.getID())) ==
                Heap.INVALID_ADDRESS) { //Check if a CID exist
            p_dataStructure.setState(ChunkState.DOES_NOT_EXIST);
        } else if (!p_acquireWriteLock || memory.switchableWriteLock(
                directEntryAddress)) { //check if a lock was received
            try {
                // #ifdef STATISTICS
                //->SOP_PUT.enter();
                // #endif /* STATISTICS */
    ...
    boolean put(final long p_chunkID, final byte[] p_data, final int p_offset, final int p_length,
        ...
        if (p_chunkID != ChunkID.INVALID_ID) { //Check if CID can be correct
            if ((directEntryAddress = cidTable.getAddressOfEntry(p_chunkID)) !=
                    Heap.INVALID_ADDRESS) { //Check if a CID exist
                if (!p_acquireWriteLock || memory.switchableWriteLock(
                        directEntryAddress)) { //check if a lock was received
                    try {
                        ...
                        entry = cidTable.directGet(directEntryAddress);
                        if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                            assert p_offset + p_length <= heap.getSizeDataBlock(entry) :
                                    "offset: " + p_offset + "\tlength: " + p_length + "\tbs: " +
                                            heap.getSizeDataBlock(entry);

                            heap.writeBytes(entry, 0, p_data, p_offset, p_length);
                            ret = true;
                        }
     ...
     * @return True if modifying the data was successful, false if
     * no chunk with the specified id exists
     */
    boolean modify(final long p_chunkID, ByteDataManipulation byteDataManipulation) {
        ...
        if (p_chunkID != ChunkID.INVALID_ID && //Check if CID can be correct
            ...
            try {
                ...
                entry = cidTable.directGet(directEntryAddress);
                if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                    int fullSize = heap.getSizeDataBlock(entry);
                    byte[] data = new byte[fullSize];
                    heap.readBytes(entry, 0, data, 0, data.length);
                    heap.writeBytes(entry, 0, byteDataManipulation.getNewData(data), 0, data.length);
                    ret = true;
                }
     ...
     * no chunk with the specified id exists
     */
    boolean modify(final DataStructure p_dataStructure,
            DataStructureManipulation<DataStructure> dataStructureManipulation) {
        long directEntryAddress;
        ...
        if (p_dataStructure.getID() == ChunkID.INVALID_ID) { //Check if CID can be correct
            ...
            try {
                ...
                if (address > Heap.INVALID_ADDRESS && !deleted) {
                    assert heap.getSizeDataBlock(entry) == p_dataStructure.sizeofObject();
        ...
        if (p_chunkID != ChunkID.INVALID_ID && //Check if CID can be correct
            ...
            try {
                ...
                if (entry != CIDTable.FREE_ENTRY && entry != CIDTable.ZOMBIE_ENTRY) {
                    ...
                    byte[] data = new byte[fullSize];
                    heap.readBytes(entry, 0, data, 0, data.length);
                    heap.writeBytes(entry, 0, chunkDataManipulation.getNewData(data, selected), 0,
                            data.length);
                    ret = true;
                }
