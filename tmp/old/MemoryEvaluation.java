...
public class MemoryEvaluation {
    ...
    public void setLocks(final boolean readLock, final boolean writeLock, final boolean disableReadLock,
            final boolean disableWriteLock, final int waitHandle) {
    ...
    private void initHeap(final boolean pinning) {
        ...
        while (memory.info.numActiveChunks > 0) {
            memory.cidTable.setState(cid, CIDTableChunkEntry.STATE_NOT_MOVEABLE, false);
    ...
    public final void accessSimulation(final double createProbability, final double removeProbability,
        ...
        String baseFilename = String.format(Locale.US, "%2.3f_%2.3f_%2.3f", createProbability, removeProbability,
                writeProbability);

        String desc = String.format(Locale.US,
                "operations: %d, threads: %d, init chunks: %d, inti size: [min: %d ,max: %d], " +
