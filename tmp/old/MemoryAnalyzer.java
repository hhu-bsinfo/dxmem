...
import static de.hhu.bsinfo.dxmem.core.CIDTable.*;
import static de.hhu.bsinfo.dxmem.core.Heap.*;
...
final public class MemoryAnalyzer {
    ...
    final public boolean analyze(final boolean p_quiet, final boolean p_dumpOnError){
        ...
        while (nextFree < Long.MAX_VALUE || nextData < Long.MAX_VALUE || nextTable < Long.MAX_VALUE) {
            ...
            //read the marker
            int marker = heap.readRightPartOfMarker(predictedAddress - SIZE_MARKER_BYTE);
            ...
            if (ERROR || predictedAddress == heap.m_baseFreeBlockList) {
                break;
            }
        }
    ...
    private void collectData() {
        ...
        m_dataBlocks.sort(Comparator.comparingLong(CIDTableChunkEntry.ADDRESS::get));

        //tables have no length fields because the size is fixed
        m_managementTables = getAllManagementTables();
        m_managementTables.sort(Comparator.comparingLong(CIDTableChunkEntry.ADDRESS::get));
    ...
    private LinkedList<Long> getAllFreeBlocks(){
        ...
        for (long i = heap.m_baseFreeBlockList; i < heap.getStatus().getSize(); i+=POINTER_SIZE) {
            tmp_address = heap.read(i, POINTER_SIZE);
            ...
            while (tmp_address != INVALID_ADDRESS){
                ...
                lfs = getSizeFromMarker(heap.readRightPartOfMarker(tmp_address-SIZE_MARKER_BYTE));
                tmp_address = heap.read(tmp_address + lfs + POINTER_SIZE, POINTER_SIZE);
            }
        }
    ...
    /**
     * Get a LinkedList with all tables saved in the local Heap
     *
    ...
    private void checkDataBlock(final long cidEntry){
        ...
        out.append(createLog(INVALID_ADDRESS < address && address < heap.m_baseFreeBlockList-SIZE_MARKER_BYTE,
                String.format("address: 0x%012X, ", address)));

        int marker = heap.readRightPartOfMarker(address-SIZE_MARKER_BYTE);
    ...
    private void checkFreeBlock(final long p_address, final boolean p_managed){
        ...
        out.append(createLog(INVALID_ADDRESS < p_address && p_address < heap.m_baseFreeBlockList-SIZE_MARKER_BYTE,
                String.format("address: 0x%X, ", p_address)));

        int marker = heap.readRightPartOfMarker(p_address -SIZE_MARKER_BYTE);
        ...
        if(marker != SINGLE_BYTE_MARKER) {
            lf = heap.read(p_address, lfs);
            out.append(createLog(lf <= heap.getStatus().getFree() || lf == 0, String.format("length field: %d, ", lf)));

            if (lf != heap.read(p_address + lf - lfs, lfs)) {
                ERROR = true;
                out.append(String.format("\033[0;31m[ERROR]>>length field differ [l: %d, r: %d] <<\033[0m, ",
                        lf, heap.read(p_address + lf - lfs, lfs)));
            }
        }

        if (marker != heap.readLeftPartOfMarker(p_address + lf)){
            ERROR = true;
            out.append(String.format("\033[0;31m[ERROR]>>marker differ [l: %d, r: %d]<<\033[0m, ",
                    marker, heap.readLeftPartOfMarker(p_address +lf)));
        }

        if(p_managed) {
            long pre = heap.readPointer(p_address + lfs);
            out.append(createLog(INVALID_ADDRESS < pre && pre < heap.m_baseFreeBlockList - SIZE_MARKER_BYTE ||
                    (pre - heap.m_baseFreeBlockList) % POINTER_SIZE == 0, String.format("pre: 0x%012X, ", pre)));

            long next = heap.readPointer(p_address + lfs + POINTER_SIZE);
            out.append(createLog(INVALID_ADDRESS <= next &&
                    p_address < heap.m_baseFreeBlockList - SIZE_MARKER_BYTE,
